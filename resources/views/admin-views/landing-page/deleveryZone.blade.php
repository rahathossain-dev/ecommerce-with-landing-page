        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    {{ \App\CPU\translate('Delevery Type | Zone Name')}}
                </div>
                <div class="card-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <form action="{{route('admin.landingPage.zone.zoneStore')}}" method="POST"
                        ...
                        <hr>
                        <button type="submit" class="btn btn-primary">{{\App\CPU\translate('Zone Name')}}</button>
                    </form>
                </div>
            </div>
        </div>
                    ...
                    <div class="flex-between justify-content-between align-items-center flex-grow-1">
                        <div>
                            <h5>{{ \App\CPU\translate('Delevery Zone')}} <span
                                    style="color: red;">({{ $zones->count() }})</span></h5>
                        </div>
                    </div>
                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-primary btn-sm edit" style="cursor: pointer;"
                                                href="{{route('admin.landingPage.zone.zoneEdit', [$zone->id])}}">
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('admin.landingPage.zone.zoneDelete')}}",
                        method: 'POST',
                        data: { id: id },
                        success: function () {
