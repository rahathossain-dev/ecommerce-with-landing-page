        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <form action="{{route('admin.landingPage.zone.deleveryChargeStore')}}" method="POST"
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group lang_form">
                                    ...
                                    <input type="hidden" name="zone_id" class="form-control" value="{{ $id }}">
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-group lang_form">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="flex-between justify-content-between align-items-center flex-grow-1">
                        <div>
                            <h5>{{ \App\CPU\translate('Delevery Zone')}} <span
                                    style="color: red;">({{ $deleveryCharges->count() }})</span></h5>
                        </div>
                    </div>
                </div>
                <div class="card-body" style="padding: 0">
                    <div class="table-responsive">
                        <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                            ...
                            <tbody>
                                @foreach($deleveryCharges as $key => $deleveryCharge)
                                    <tr>
                                        <td class="text-center">{{$deleveryCharge->id}}</td>
                                        <td>{{$deleveryCharge->name}}</td>
                                        <td>
                                            <a class="btn btn-primary btn-sm edit" style="cursor: pointer;"
                                                href="{{route('admin.landingPage.zone.deleveryChargeEdit', [$deleveryCharge->id])}}">
                                                <i class="tio-edit"></i> Edit
                                            </a>
                                            <a class="btn btn-danger btn-sm delete" style="cursor: pointer;"
                                                id="{{$deleveryCharge->id}}">
        ...
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('admin.landingPage.zone.deleveryChargeDelete')}}",
                        method: 'POST',
                        data: { id: id },
                        success: function () {
